@ppxf_nifs.pro
@ppxf_gnirs.pro
PRO VORONOI_WDISP,GNIRS=gnirs,min_sn=min_sn,CHECK=check
...
FOR i=0,nbins-1 DO BEGIN
    ...
    IF (KEYWORD_SET(GNIRS)) THEN BEGIN
        ppxf_gnirs,lambda,REFORM(bin_spectra[i,*]),REFORM(bin_var[i,*]),bin_fwhm[i],outrv=vel,outdisp=disp,outsol=sol,outerror=err,/doerror,binnum=i,initvel=initvel
        spimage[ind]=sol[7]
        spbin[i]=sol[7]
        spdispbin[i]=sol[8]
        lumimage[ind]=sol[9]
        ...
        chiimage[ind]=sol[6]
        chibin[i]=sol[6]
        printf,1,i,xoutbin[i],youtbin[i],npixels[i],measured_sn[i],sn[i],sol[6],sol[0],err[0],err[1],sol[1],err[2],err[3],sol[2],err[4],err[5],sol[3],err[6],err[7],sol[7],err[8],err[9],sol[9],err[10],err[11],FORMAT='(I4,2F7.3,I5,21F8.2)'
;       printf,1,i,xoutbin[i],youtbin[i],npixels[i],measured_sn[i],sn[i],sol[6],sol[0],sol[1],sol[2],sol[3],sol[7],sol[9],FORMAT='(I4,2F7.3,I5,7F8.2)'
    ENDIF ELSE BEGIN
...
ENDIF
binfile='vor_out/nifs_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'_bin.fits'
IF (KEYWORD_SET(GNIRS)) THEN binfile='vor_out/gnirs_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'_bin.fits'
WRITEFITS,binfile,binimage    

velfile='vor_out/vel_nifs_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'.fits'
IF (KEYWORD_SET(GNIRS)) THEN velfile='vor_out/vel_gnirs_sn'+STRTRIM(FIX(min_sn),2)+'.fits'
WRITEFITS,velfile,velimage
dispfile='vor_out/disp_nifs_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'.fits'
IF (KEYWORD_SET(GNIRS)) THEN dispfile='vor_out/disp_gnirs_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'.fits'
...
outpsfile='vor_out/voronoi_nifs_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'.ps'
IF (KEYWORD_SET(GNIRS)) THEN outpsfile='vor_out/voronoi_gnirs_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'.ps'
