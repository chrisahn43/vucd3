@ppxf_wallace.pro
PRO VORONOI_WALLACE,MIN_SN=min_sn
...
;before running need to create a vor_out subdirectory

ifu=READFITS(infile,head,ext=1)
var=READFITS(infile,ext=2)
;fwhm=FLTARR(size[0],size[1])
;fwhm[*]=4.1
fwhm=READFITS(fwhmfile)
;subsection with good data
ifu=ifu[19:65,19:65,*]
var=var[19:65,19:65,*]
fwhm=fwhm[19:65,19:65]
size=SIZE(ifu,/DIM)
;tag for outputs
tag='best9'
...
makex,ifu,x,y
;Center      20.2577      22.8544

outplotfile='vor_out/wallace_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'.ps'
!P.THICK=2
!P.CHARTHICK=2
!X.THICK=2
!Y.THICK=2
...
chibin=FLTARR(nbins)
outfile='vor_out/wallace_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'.dat' 
!P.MULTI=[0,1,2]
...
FOR i=0,nbins-1 DO BEGIN
    ...
    IF (nbadspec GT 100) THEN BEGIN
        ...
        plot,lambda,REFORM(bin_spectra[i,*])
        printf,1,i,xoutbin[i],youtbin[i],npixels[i],measured_sn[i],sn[i],REPLICATE(-999.,20),FORMAT='(I4,2F7.3,I5,22F9.3)'
        GOTO,SKIP
    ENDIF ELSE BEGIN
    ppxf_wallace,lambda,REFORM(bin_spectra[i,*]),REFORM(bin_var[i,*]),bin_fwhm[i],outrv=vel,outdisp=disp,outsol=sol,binnum=i,initvel=initvel,nodispchi=nodispchi,outerror=err,/doerror
    spimage[ind]=sol[7]
    spbin[i]=sol[7]
    spdispbin[i]=sol[8]
    lumimage[ind]=sol[9]
...
WRITEFITS,'vor_out/wallace_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'_mg.fits',mgimage
WRITEFITS,'vor_out/wallace_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'_brg.fits',brgimage
WRITEFITS,'vor_out/wallace_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'_na.fits',naimage
WRITEFITS,'vor_out/wallace_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'_ca.fits',caimage
WRITEFITS,'vor_out/wallace_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'_co.fits',coimage
WRITEFITS,'vor_out/wallace_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'_sn.fits',snimage
WRITEFITS,'vor_out/wallace_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'_sp.fits',spimage
WRITEFITS,'vor_out/wallace_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'_lum.fits',lumimage

binfile='vor_out/wallace_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'_bin.fits'
WRITEFITS,binfile,binimage    
velfile='vor_out/vel_wallace_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'.fits'
WRITEFITS,velfile,velimage
dispfile='vor_out/disp_wallace_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'.fits'
