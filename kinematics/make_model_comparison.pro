...
;make the 255th color gray
    ...
    imgunder,useimage
    xyouts,[-0.95],[0.9],['Data'],charsize=1.0,color=235
    xyouts,[0.8],[0.8],['a'],charsize=1.5
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    CONTOUR, cont,xcoord,ycoord, levels=levels,/fill,/xs,/ys,/nodata,charsize=1.0,xtitle='RA Offset ["]',ytitle='Dec Offset ["]',xminor=2,/iso,xmargin=[9,4],ymargin=[0.5,3.5],xcharsize=0.0001
;make the 255th color gray
    LOADCT,33,/silent
    R_CURR[255]=150 & G_CURR[255]=150 & B_CURR[255]=150
    R_CURR[0]=0 & G_CURR[0]=0 & B_CURR[0]=0
...
;first do velocities   
    ...
    high=WHERE(velimage GT maxlev,nhigh)
    IF (nhigh GT 0) THEN velcontimage[high]=!VALUES.F_NAN
    CONTOUR, cont,xcoord,ycoord, levels=klevels,/fill,/xs,/ys,/nodata,charsize=1.0,xtitle='RA Offset ["]',ytitle='Dec Offset ["]',xminor=2,/iso,xmargin=[9,4],ymargin=[3.5,0.5],xcharsize=0.00001
    pixelscale=SIN(1.0D0/3600./180.*!DPI)*10.^((dms[i]+5.)/5.)
    print,galaxies[i],pixelscale
    xmin=MIN(xcoord)*pixelscale
    xmax=MAX(xcoord)*pixelscale
...
;make the 255th color gray
    ...
    useimage=REBIN(velcolimage,size[0]*5,size[1]*5,/sample)
    imgunder,useimage
    xyouts,[-0.95],[0.9],['Model'],charsize=1.0,color=235
    xyouts,[0.8],[0.8],['c'],charsize=1.5
    axis,yaxis=0,ychars=0.00000001,/ysty

    contour,cont,xcoord,ycoord,/overplot,levels=klevels,thick=4
    axis,xaxis=0,xcharsize=1.0,xtitle='RA Offset ["]',xrange=[!X.CRANGE[1],!X.CRANGE[0]],/xsty
    COLORBAR,range=[min(vellevels),max(vellevels)+dellev],format='(I4)',/vertical,right=1,divisions=5,charsize=1.0,title='!C Relative Velocity [km/s]',POSITION=[!X.WINDOW[1],!Y.WINDOW[0],!X.WINDOW[1]+0.03,!Y.WINDOW[1]]
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    CONTOUR, cont,xcoord,ycoord, levels=levels,/fill,/xs,/ys,/nodata,charsize=1.0,xtitle='RA Offset ["]',ytitle='Dec Offset ["]',xminor=2,/iso,xmargin=[9,4],ymargin=[3.5,0.5],xchars=0.00001
;make the 255th color gray
    LOADCT,33,/silent
    R_CURR[255]=150 & G_CURR[255]=150 & B_CURR[255]=150
    R_CURR[0]=0 & G_CURR[0]=0 & B_CURR[0]=0
