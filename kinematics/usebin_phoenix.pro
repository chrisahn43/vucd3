@ppxf_phoenix.pro
PRO USEBIN_PHOENIX,MIN_SN=min_sn
...
outplotfile='vor_out/usebin_'+tag+'_phoenix_sn'+STRTRIM(FIX(min_sn),2)+'.ps'
!P.THICK=2
!P.CHARTHICK=2
!X.THICK=2
!Y.THICK=2
...
chibin=FLTARR(nbins)
outfile='vor_out/usebin_'+tag+'_phoenix_sn'+STRTRIM(FIX(min_sn),2)+'.dat' 
!P.MULTI=[0,1,2]
...
;        binimage[ind]=i
        ...
        GOTO,SKIP
    ENDIF ELSE BEGIN
    PPXF_PHOENIX,lambda,REFORM(bin_spectra[i,*]),REFORM(bin_var[i,*]),bin_fwhm[i],outrv=vel,outdisp=disp,outsol=sol,binnum=ibin[i],initvel=initvel,nodispchi=nodispchi,outerror=err,bestfit=bestfit,loglamout=loglamout,galaxy=galaxy,/doerror
...
MWRFITS,specfit,'vor_out/usebin_'+tag+'_phoenix_sn'+STRTRIM(FIX(min_sn),2)+'_specfit.fits',/CREATE
WRITEFITS,'vor_out/usebin_'+tag+'_phoenix_sn'+STRTRIM(FIX(min_sn),2)+'_mg.fits',mgimage
WRITEFITS,'vor_out/usebin_'+tag+'_phoenix_sn'+STRTRIM(FIX(min_sn),2)+'_brg.fits',brgimage
WRITEFITS,'vor_out/usebin_'+tag+'_phoenix_sn'+STRTRIM(FIX(min_sn),2)+'_na.fits',naimage
WRITEFITS,'vor_out/usebin_'+tag+'_phoenix_sn'+STRTRIM(FIX(min_sn),2)+'_ca.fits',caimage
WRITEFITS,'vor_out/usebin_'+tag+'_phoenix_sn'+STRTRIM(FIX(min_sn),2)+'_co.fits',coimage
WRITEFITS,'vor_out/usebin_'+tag+'_phoenix_sn'+STRTRIM(FIX(min_sn),2)+'sn.fits',snimage
WRITEFITS,'vor_out/usebin_'+tag+'_phoenix_sn'+STRTRIM(FIX(min_sn),2)+'_sp.fits',spimage
WRITEFITS,'vor_out/usebin_'+tag+'_phoenix_sn'+STRTRIM(FIX(min_sn),2)+'_lum.fits',lumimage

binfile='vor_out/usebin_'+tag+'_phoenix_sn'+STRTRIM(FIX(min_sn),2)+'_bin.fits'
velfile='vor_out/vel_usebin_'+tag+'_phoenix_sn'+STRTRIM(FIX(min_sn),2)+'.fits'
WRITEFITS,velfile,velimage
dispfile='vor_out/disp_usebin_'+tag+'_phoenix_sn'+STRTRIM(FIX(min_sn),2)+'.fits'
...
;outpsfile='vor_out/voronoi_phoenix_'+tag+'_sn'+STRTRIM(FIX(min_sn),2)+'.ps'
